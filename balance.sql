...
BEGIN
SELECT DISTINCT Timestamp from history WHERE Timestamp >= fromDate and Timestamp <= toDate;
END$$
DELIMITER ;
...
BEGIN
SELECT Close FROM history WHERE Timestamp=closeDate AND assetID=id;
END$$
DELIMITER ;
